...
// ── Controls ─────────────────────────────────────────────────
// controls ใช้ yawObject/pitchObject hierarchy
// ต้อง add yawObject เข้า scene แทนการ set camera.position ตรงๆ
const controls = new GalleryControls(camera, canvas);
controls.position.set(0, 1.65, -1.5); // ตำแหน่งเริ่มต้น
scene.add(controls._yawObject);       // add rig เข้า scene
...
audioBtn.addEventListener('click', async () => {
  if (audioOn) { audio.stop(); audioBtn.classList.remove('active'); audioOn = false; }
  else { await audio.start(); audioBtn.classList.add('active'); audioOn = true; }
});
